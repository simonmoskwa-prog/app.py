...
def get_data_with_info(ticker):
    ...
    try:
        ...
        # Lokale Zeitprüfung auf Basis deutscher Marktzeiten
        now_local = datetime.datetime.now()
        is_weekday = now_local.weekday() < 5  # Mo-Fr

        if is_weekday:
            current_time = now_local.time()
            # Europäische Märkte (Xetra, WSE, Euronext etc.) -> 09:00 bis 17:30 Uhr
            if any(suffix in ticker for suffix in ["DE", "STX", "DAX", "WA", "PA", "F"]):
                is_open = datetime.time(9, 0) <= current_time <= datetime.time(17, 30)
            # US-Märkte (NYSE, Nasdaq, S&P 500) -> 15:30 bis 22:00 Uhr
            else:
                is_open = datetime.time(15, 30) <= current_time <= datetime.time(22, 0)
        else:
            is_open = False
