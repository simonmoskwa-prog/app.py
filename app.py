    ...
    with st.container(border=True):
        ...
        if lt_query_to_run:
            ...
            if lt_data_ok:
                ...
                with st.expander("ℹ️ Details & Methode zur Modellberechnung anzeigen"):
                    st.markdown(f"""
                    * **Angenommenes Gewinnwachstum p. a.:** `{format_metric_pct(val_res['growth_used'])}` *(Deckelung bei max. 15% zur Vermeidung unrealistischer Zukunftsannahmen)*
                    * **Erwarteter EPS in {lt_forecast_years} Jahren:** `{format_metric_money(val_res['future_eps'], lt_curr)}`
                    * **Angenommenes KGV in {lt_forecast_years} Jahren:** `{val_res['assumed_future_pe']:.1f}`
                    * **Diskontierung:** {lt_min_return_pct}% p. a. über {lt_forecast_years} Jahre
                    """)
            else:
                st.error(f"Für das Kürzel/Unternehmen '{lt_query_to_run}' konnten keine fundamentalen Datensätze abgerufen werden.")
