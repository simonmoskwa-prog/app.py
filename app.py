...
def is_market_open(ticker: str) -> bool:
    ...
    # 2. Erweiterte Handelszeit über deutsche Handelsplätze
    extended_open = (
        now_berlin.weekday() < 5
...
# ---------------------------------------------------------
# CHART-LAYOUT & RENDERING (FIXED)
# ---------------------------------------------------------
def apply_current_period_view(fig, plot_df):
    """
    Stellt sicher, dass die x-Achse alle Monate sauber anzeigt
    und nach dem aktuellsten Datenpunkt genügend Platz lässt.
    """
    if plot_df.empty:
        return fig
    ...
    # 14 Tage Padding nach rechts, damit auch der aktuellste Monat/Balken voll sichtbar ist
    range_end = last_date + pd.Timedelta(days=14)

    fig.update_xaxes(
        type="date",
        tickformat="%b %Y",
        dtick="M1",  # FIX: Zeigt JEDEN Monat an (auch September)
...
def render_chart(fig, height=450):
    """
    Rendert die Grafik direkt über Streamlit, um das Vollbild-Icon 
    sowie die responsive Skalierung auf Mobilgeräten zu aktivieren.
    """
    fig.update_layout(
        height=height,
        autosize=True,
        margin=dict(l=55, r=20, t=30, b=30),  # Genug Abstand links für Y-Achsenbeschriftungen
    ...
    # Y-Achsen Anpassung gegen Überlappung der Texte auf mobilen Geräten
    fig.update_yaxes(automargin=True, title_standoff=8)

    # Native Streamlit-Ausgabe ermöglicht echtes Vollbild über das Icon oben rechts
