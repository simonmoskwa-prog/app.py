...
for idx, item in enumerate(sector_results):
    ...
    with col:
        with st.container(border=True):
            ...
            # Nebeneinander-Layout: Expander links, Hilfebutton rechts oben direkt neben dem Expander-Header
            exp_col1, exp_col2 = st.columns([0.88, 0.12])
            
            with exp_col2:
                if st.button("❓ Hilfeseite", key=f"help_btn_{item['ticker']}_{idx}", help="Öffne das Glossar & die Regelwerk-Erklärung zu den TOP-Kandidaten."):
                    show_glossary_dialog()
                        ...
                        st.dataframe(
                            ...
                            column_config={
                                "Top": st.column_config.TextColumn(
                                    "Top ❓",
                                    help="⭐ TOP zeigt an, dass die Aktie alle 4 Kriterien für ein perfektes Stage 2 Setup erfüllt (siehe Hilfeseite)."
                                )
                            }
                        )
                    else:
