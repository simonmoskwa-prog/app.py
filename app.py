...
# ---------------------------------------------------------
# ROBUSTE MANSFIELD RS BERECHNUNG (IMMER EIN WERT)
# ---------------------------------------------------------
def calculate_mansfield_rs(asset_df, benchmark_df):
    ...
    # Zusammenführen und fehlende Lücken lückenlos auffüllen
    combined = pd.concat([asset_close, bench_close], axis=1, keys=["Asset", "Bench"]).ffill().bfill()

    base_rs = combined["Asset"] / combined["Bench"]
    
    # Rolling 52 mit min_periods=1 stellt sicher, dass immer Werte berechnet werden
    rs_sma52 = base_rs.rolling(window=52, min_periods=1).mean()
    
    mansfield_series = ((base_rs / rs_sma52) - 1) * 100
    
    # Reindexieren auf Datensatz der Aktie
    mansfield_series = mansfield_series.reindex(asset_close.index).ffill().bfill().fillna(0)
    return mansfield_series
