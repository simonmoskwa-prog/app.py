...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Trading & Invest Dashboard | Pulse Trading",
    page_icon="📈",
    layout="wide",
...
# ---------------------------------------------------------
# LOGO (S.M. TRADING) — wird sowohl auf der Login-Seite als
# auch im Header der Hauptseite verwendet. Muss vor dem
# Login-Screen geladen werden, da dieser die App bei fehlender
# Anmeldung sofort per st.stop() beendet.
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# WICHTIG: Es stehen keinerlei Zugangsdaten (Benutzername, Passwort,
# Anzeigename, Rolle) mehr im Quellcode. Alles wird ausschließlich über
# Streamlit Secrets verwaltet (lokal: .streamlit/secrets.toml, in der
# Cloud: die "Secrets"-Einstellung der App). Diese Datei/Einstellung
# darf NIEMALS ins Git-Repository eingecheckt werden
# (.streamlit/secrets.toml gehört in die .gitignore).
#
# Erwartetes Format in secrets.toml:
#
# [[users]]
# username = "admin"
...
#
# [[users]]
# username = "marcin.plonka"
# password = "ein-anderes-sicheres-passwort"
# display_name = "Marcin Płonka"
# role = "user"
#
# (für jeden weiteren Nutzer einen eigenen [[users]]-Block ergänzen)
...
def load_users():
    ...
    for entry in secret_users:
        u_key = str(entry.get("username", "")).strip().lower()
        if not u_key or "password" not in entry:
            continue
        display_name = entry.get("display_name", u_key.capitalize())
        users_db[u_key] = {
            "password": hash_password(str(entry["password"])),
            "display_name": display_name,
            "welcome_msg": f"Willkommen {display_name}",
    ...
    with col2:
        ...
        with st.container(border=True):
                ...
                st.markdown(
                    ...
                    <div style="display: flex; justify-content: center; margin-bottom: 12px;">
                        <img src="data:image/png;base64,{logo_b64}"
                             style="width: 140px; height: auto; object-fit: contain; border-radius: 12px;">
                    </div>
                    """,
            ...
            if not st.session_state["users_db"]:
                st.warning(
                    "Es sind noch keine Zugangsdaten konfiguriert. Bitte in "
                    "`.streamlit/secrets.toml` (lokal) bzw. in den App-Secrets "
                    "(Streamlit Cloud) mindestens einen `[[users]]`-Eintrag anlegen."
                )
...
<style>
    ...
    .metric-card-good { border-color: rgba(74, 222, 128, 0.4); background: rgba(74, 222, 128, 0.07); }
    .metric-card-bad { border-color: rgba(248, 113, 113, 0.4); background: rgba(248, 113, 113, 0.06); }
    .metric-card-neutral { border-color: rgba(148, 163, 184, 0.3); background: rgba(148, 163, 184, 0.05); }
    .metric-card-label {
        font-size: 0.72rem;
        color: #94a3b8;
        font-weight: 600;
    ...
    .tag-good { background: rgba(74, 222, 128, 0.18); color: #4ade80; }
    .tag-bad { background: rgba(248, 113, 113, 0.18); color: #f87171; }
    .tag-neutral { background: rgba(148, 163, 184, 0.18); color: #94a3b8; }
...
def render_metric_card(label, value_str, status="neutral", tag_text=None):
    """Baut eine kleine, farblich codierte Kennzahlen-Karte (Ampel-Prinzip) als HTML."""
    tag_html = f"<span class='metric-card-tag tag-{status}'>{tag_text}</span>" if tag_text else ""
    return (
        f"<div class='metric-card metric-card-{status}'>"
...
def classify_growth_metric(value, good_threshold=0.10):
    """Ordnet eine Wachstums-/Renditekennzahl einer Ampel-Kategorie zu."""
    if value is None:
        return "neutral", "n. v."
    if value >= good_threshold:
        return "good", "solide"
    if value >= 0:
        return "neutral", "moderat"
    return "bad", "rückläufig"
...
with header_col2:
    st.markdown("<div class='dashboard-title'>Trading & Invest Dashboard</div>", unsafe_allow_html=True)
    st.markdown("<div class='dashboard-subtitle'>by <b>Pulse Trading</b> | Kurzfristige Stage-Analyse & Langfristige Wertanalyse in einem Tool</div>", unsafe_allow_html=True)
...
with tab_long:
    ...
    st.markdown(
        ...
        <div class="disclaimer-box" style="margin-bottom: 18px;">
            ...
            Alle Berechnungen basieren auf historischen und ggf. lückenhaften Daten; einzelne Kennzahlen können je nach
            Datenverfügbarkeit als "n. v." ausgewiesen werden.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with st.container(border=True):
        ...
        if lt_query_to_run:
            ...
            # --- Schritt 1: Kursdaten laden (harte Voraussetzung) ---
            lt_data_ok = False
            try:
                with st.spinner(f"Lade Kursdaten für '{lt_query_to_run}' (Handelszeichen: {lt_resolved_ticker})..."):
            ...
            if not lt_data_ok:
                st.warning(f"Für '{lt_query_to_run}' (gefundenes Handelszeichen: '{lt_resolved_ticker}') konnten keine Kursdaten gefunden werden. Bitte Eingabe prüfen.")
            else:
                # --- Schritt 2: Fundamentalkennzahlen laden (weich - fehlende Werte sind ok) ---
                try:
                    lt_metrics = compute_quality_growth_metrics(lt_resolved_ticker)
                except Exception:
                    lt_metrics = {
                        "current_price": None, "currency": lt_curr, "eps_ttm": None, "trailing_pe": None,
                        "shares_out": None, "roic_avg": None, "eps_cagr": None, "sales_cagr": None,
                        "equity_cagr": None, "fcf_cagr": None, "years_available": 0,
                    }
                ...
                try:
                    ...
                    lt_valuation = {
                        "growth_used": None, "future_eps": None, "assumed_future_pe": None,
                        "target_value": None, "safety_value": None, "payback_years": None,
                        "verdict": "nicht bewertbar (unzureichende Datenlage)", "verdict_class": "value-neutral-badge",
                    }
                ...
                # --- Bewertungs-Spanne als visuelle Einordnung ---
                if lt_valuation.get("safety_value") and lt_valuation.get("target_value") and lt_price:
                    try:
                        with st.container(border=True):
                            st.markdown("##### 📏 Einordnung des aktuellen Kurses")
                            lt_safety_v = lt_valuation["safety_value"]
                            lt_target_v = lt_valuation["target_value"]
                            lt_axis_max = max(lt_price, lt_target_v) * 1.15
                            lt_axis_min = 0

                            fig_range = go.Figure()
                            fig_range.add_shape(type="rect", x0=lt_axis_min, x1=lt_safety_v, y0=0, y1=1,
                                                 fillcolor="rgba(74, 222, 128, 0.18)", line_width=0)
                            fig_range.add_shape(type="rect", x0=lt_safety_v, x1=lt_target_v, y0=0, y1=1,
                                                 fillcolor="rgba(251, 191, 36, 0.14)", line_width=0)
                            fig_range.add_shape(type="rect", x0=lt_target_v, x1=lt_axis_max, y0=0, y1=1,
                                                 fillcolor="rgba(248, 113, 113, 0.14)", line_width=0)

                            fig_range.add_trace(go.Scatter(
                                x=[lt_price], y=[0.5], mode="markers+text",
                                marker=dict(size=22, color="#38bdf8", line=dict(color="#ffffff", width=2)),
                                text=["Aktueller Kurs"], textposition="top center",
                                textfont=dict(color="#e2e8f0", size=12), showlegend=False,
                            ))
                            fig_range.add_vline(x=lt_safety_v, line_dash="dot", line_color="#4ade80",
                                                 annotation_text="Sicherheitswert", annotation_position="bottom")
                            fig_range.add_vline(x=lt_target_v, line_dash="dash", line_color="#fbbf24",
                                                 annotation_text="Zielwert", annotation_position="bottom")

                            fig_range.update_xaxes(range=[lt_axis_min, lt_axis_max], showgrid=False,
                                                    title=f"Kurs ({lt_currency})")
                            fig_range.update_yaxes(visible=False, range=[0, 1])
                            fig_range.update_layout(
                                height=170, margin=dict(l=20, r=20, t=10, b=40),
                                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                                showlegend=False,
                            )
                            st.plotly_chart(fig_range, use_container_width=True, config={"displayModeBar": False})
                            st.caption("🟢 unter Sicherheitswert · 🟡 zwischen Sicherheitswert & Zielwert · 🔴 über Zielwert")
                    except Exception:
                        pass
                ...
                roic_status, roic_tag = classify_growth_metric(lt_metrics["roic_avg"])
                eps_status, eps_tag = classify_growth_metric(lt_metrics["eps_cagr"])
                sales_status, sales_tag = classify_growth_metric(lt_metrics["sales_cagr"])
                equity_status, equity_tag = classify_growth_metric(lt_metrics["equity_cagr"])
                fcf_status, fcf_tag = classify_growth_metric(lt_metrics["fcf_cagr"])

                card_cols = st.columns(5)
                card_defs = [
                    ("Ø KAPITALRENDITE (ROIC)", format_metric_pct(lt_metrics["roic_avg"]), roic_status, roic_tag),
                    ("GEWINNWACHSTUM (Ø P. A.)", format_metric_pct(lt_metrics["eps_cagr"]), eps_status, eps_tag),
                    ("UMSATZWACHSTUM (Ø P. A.)", format_metric_pct(lt_metrics["sales_cagr"]), sales_status, sales_tag),
                    ("EIGENKAPITALWACHSTUM (Ø P. A.)", format_metric_pct(lt_metrics["equity_cagr"]), equity_status, equity_tag),
                    ("FREIER CASHFLOW (Ø P. A.)", format_metric_pct(lt_metrics["fcf_cagr"]), fcf_status, fcf_tag),
                ]
                for col, (label, value_str, status, tag) in zip(card_cols, card_defs):
                    with col:
                        st.markdown(render_metric_card(label, value_str, status, tag), unsafe_allow_html=True)

                st.write("")
                card_cols2 = st.columns(3)
                with card_cols2[0]:
                    st.markdown(render_metric_card(
                        "IM MODELL ANGENOMMENES WACHSTUM",
                        format_metric_pct(lt_valuation["growth_used"]), "neutral"
                    ), unsafe_allow_html=True)
                with card_cols2[1]:
                    pe_val = f"{lt_valuation['assumed_future_pe']:.1f}" if lt_valuation["assumed_future_pe"] else "n. v."
                    st.markdown(render_metric_card("ANGENOMMENES KÜNFTIGES KGV", pe_val, "neutral"), unsafe_allow_html=True)
                with card_cols2[2]:
                    payback_val = f"{lt_valuation['payback_years']} Jahre" if lt_valuation["payback_years"] else "> 30 Jahre"
                    st.markdown(render_metric_card("RECHNERISCHE AMORTISATIONSDAUER", payback_val, "neutral"), unsafe_allow_html=True)

                st.write("")

                # --- Kursverlauf mit Zielwert-Linien ---
                try:
                    with st.container(border=True):
                        st.markdown("##### 📈 Kursverlauf mit Modell-Zielwerten")
                        lt_plot_df = lt_df.tail(208)
                        fig_lt = go.Figure()
                        fig_lt.add_trace(
                            go.Scatter(
                        ...
                        if lt_valuation.get("target_value"):
                            fig_lt.add_hline(
                        ...
                        if lt_valuation.get("safety_value"):
                            fig_lt.add_hline(
                    ...
                    st.info("Für diesen Titel konnte aktuell kein Kursverlauf-Chart erzeugt werden.")
                ...
                with st.expander("❓ Wie kommen diese Werte zustande? (Modell-Erläuterung)"):
                    ...
                    Alle Angaben beruhen auf historischen, öffentlich verfügbaren Daten und stellen keine Garantie oder Prognose der künftigen Kursentwicklung dar. Fehlende oder unvollständige Datenpunkte werden als "n. v." (nicht verfügbar) ausgewiesen und fließen nicht verfälschend in die Berechnung ein.
                    """)
