...
# ---------------------------------------------------------
# BENUTZER-VERWALTUNG (PERSONALISIERTE ZUGÄNGE)
# ---------------------------------------------------------
USERS = {
    "admin": {
        "password": "pulse2026",
...
def render_login_screen():
    ...
    with col2:
        ...
        with st.container(border=True):
            ...
            st.caption("Bitte Zugangsdaten eingeben, um auf das Stage-Analysis Dashboard zuzugreifen.")
            ...
            if login_btn:
                if user_input in USERS and USERS[user_input]["password"] == pwd_input:
                    ...
                    st.success(f" Erstanmeldung erfolgreich: {st.session_state['welcome_msg']}!")
                    st.rerun()
                else:
...
# Personalisiere die Sidebar für den angemeldeten Nutzer
with st.sidebar:
    st.markdown("### 👤 Benutzer-Session")
    st.markdown(f"**{st.session_state['welcome_msg']}**")
    st.caption(f"Konto: {st.session_state['username']}")
    st.write("")
...
# ---------------------------------------------------------
# OPTISCH HOCHWERTIGER HINWEISTEXT (LIVE UPDATES)
# ---------------------------------------------------------
st.markdown(
    f"""
...
# PERMANENT SICHTBARE RECHTS-BUTTONS IM FOOTER
