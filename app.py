...
def get_ticker_from_name(query):
    """
    Sucht nach dem passenden Ticker-Symbol für einen eingegebenen Unternehmensnamen oder Ticker.
    """
    query = query.strip()
    if not query:
        return None
...
with st.container(border=True):
    ...
    with search_col1:
        search_query = st.text_input(
            "Aktienname oder Ticker eingeben (z.B. Dino Polska, Apple, NVDA, TSLA, MSFT):",
            value="Dino Polska",
    ...
    if search_query:
        # Methode 1: Ticker via API auflösen
