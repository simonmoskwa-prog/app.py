...
# ---------------------------------------------------------
# BENUTZER-VERWALTUNG (DYNAMISCH IM SESSION_STATE)
# ---------------------------------------------------------
# Standard-Benutzer initialisieren, falls noch nicht im Session State vorhanden
...
def render_login_screen():
    ...
    with col2:
        ...
        with st.container(border=True):
            ...
            pwd_input = st.text_input("Passwort", type="password", key="login_pwd").strip()
...
for idx, item in enumerate(sector_results):
    ...
    with col:
        with st.container(border=True):
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df["Close"], mode="lines", name=f"Kurs ({item['currency']})", line=dict(color="#38bdf8", width=2.5), fill='tozeroy', fillcolor='rgba(56, 189, 248, 0.05)'))
            fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df["SMA30"], mode="lines", name="30W-SMA", line=dict(color="#fbbf24", width=2, dash="dash")))

            fig.update_layout(
                height=240, margin=dict(l=10, r=20, t=15, b=25), showlegend=True,
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", hovermode="x unified",
                xaxis=dict(type="date", tickformat="%b %Y", dtick="M3", showgrid=True, gridcolor="rgba(255,255,255,0.04)"),
                yaxis=dict(visible=True, range=y_range_sec, showgrid=True, gridcolor="rgba(255,255,255,0.05)")
            )
