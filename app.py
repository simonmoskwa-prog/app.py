...
st.set_page_config(
    page_title="Weinstein Stage-Analysis Dashboard | Pulse Trading",
    layout="wide",
)


# Funktion zum scharfen Laden des Logos als Base64
...
# Header mit hochauflösendem Logo & Titel
    ...
    st.markdown(
        ...
        <div style="display: flex; align-items: center; justify-content: center; padding-top: 5px;">
            <img src="data:image/png;base64,{logo_b64}" 
                 style="width: 110px; height: auto; object-fit: contain; image-rendering: -webkit-optimize-contrast; image-rendering: crisp-edges;">
        </div>
        """,
...
with header_col2:
  ...
  st.caption(
      "Markt → Sektor → Top-Aktien | Basierend auf Stan Weinsteins"
      " Phasenanalyse (Wochencharts, 30-Wochen-SMA & Mansfield Relative"
      " Strength)"
  )
...
for idx, item in enumerate(sector_results):
  ...
  with col:
    with st.container(border=True):
      ...
      st.markdown(
          f"<span style='padding:3px 8px; border-radius:5px;"
      ...
      st.write(
          f"**Mansfield-RS:** `{item['m_rs']:+.2f}` | **30W-MA Steigung:**"
          f" `{item['slope']:.2f}%`"
      )
      ...
      with st.expander("🔍 Top-Aktien im Stage-2-Setup anzeigen"):
        ...
        if stock_data:
          ...
          if selected_ticker:
            ...
            st.caption(
                f"Stan Weinstein Phasen-Chart: **{selected_name}"
                f" ({selected_ticker})**"
            )
            st.plotly_chart(fig_stock, use_container_width=True)
        else:
          st.info(
              "Aktuell keine Einzelaktie in diesem Sektor mit perfektem"
              " Stage-2-Setup."
          )
