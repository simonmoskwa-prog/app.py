...
# ---------------------------------------------------------
@st.dialog("📩 Contact & Trading Community")
def show_contact_dialog():
    st.markdown("### Get in Touch")
    st.write(
        "If you are interested in stocks, chart analysis, or joining our private trading exchange, "
        "feel free to contact me at **[simon.moskwa@outlook.de](mailto:simon.moskwa@outlook.de)**."
    )
    
    st.markdown("---")
    st.markdown("#### 💬 Community Exchange")
    st.write(
        "We also offer a private community exchange via WhatsApp for passionate traders. "
        "Access to the group is granted individually upon email request."
    )

    st.markdown("---")
    st.warning(
        "**Legal Notice / Disclaimer:**\n\n"
        "All information, metrics, and chart analyses provided are strictly for educational and informational purposes only. "
        "They do **not** constitute financial advice, investment recommendations, or an offer to buy or sell securities. "
        "Trading financial instruments carries substantial risks of loss."
    )
...
for idx, item in enumerate(sector_results):
    ...
    with col:
        with st.container(border=True):
            ...
            # Kennzahlen-Zeile (HTML-Syntaxfehler behoben)
            rs_color = "#28a745" if item["m_rs"] > 0 else "#dc3545"
            st.markdown(
