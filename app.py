...
# ---------------------------------------------------------
# BÖRSENZEITEN-PRÜFUNG (FIX: echte Zeitzonen + maximale Handelszeit)
# ---------------------------------------------------------
def is_market_open(ticker: str) -> bool:
    """
    Prüft, ob für den Ticker GERADE gehandelt werden kann.

    Berücksichtigt wird die MAXIMALE Handelszeit:
    1. Die Kernhandelszeit der Heimatbörse (NYSE/Nasdaq bzw. europäische Börse)
    2. UND zusätzlich die erweiterte deutsche Handelszeit über Handelsplätze
       wie Tradegate, gettex oder Lang & Schwarz (ca. 08:00-22:00 Uhr MEZ),
       über die auch US-Werte (Apple, Exxon etc.) den Großteil des Tages
       in Deutschland/Europa gehandelt werden können.

    Ist eine der beiden Handelszeiten aktiv, gilt die Aktie als "geöffnet".
    """
    ...
    # 1. Kernhandelszeit der jeweiligen Heimatbörse
    if is_european:
        core_open = (
    ...
    # 2. Erweiterte Handelszeit über deutsche Handelsplätze (gilt für alle Werte,
    #    da dort z.B. auch US-Aktien fast ganztägig handelbar sind)
    extended_open = (
        now_berlin.weekday() < 5
...
def get_data_with_info(ticker):
    ...
    try:
        ...
        # FIX: echte Zeitzonen statt angenommener deutscher Ortszeit
...
# ---------------------------------------------------------
# CHART-LAYOUT: aktuelle Periode (Monat/Woche) immer sichtbar
# ---------------------------------------------------------
def apply_current_period_view(fig, plot_df):
    """
    Stellt sicher, dass die x-Achse bis zum aktuellsten Datenpunkt
    (aktuelle Woche/aktueller Monat) reicht und markiert diesen mit
    einer 'Heute'-Linie, statt das Chart an einem älteren Quartals-
    Tick enden zu lassen.
    """
    if plot_df.empty:
        return fig
    ...
    range_end = last_date + pd.Timedelta(days=6)
...
def render_chart(fig, height=380):
    """
    Rendert eine Plotly-Grafik über eine eigene HTML-Komponente.
    Dadurch kann die Plotly-Toolbar (Kamera-/Zoom-/Pan-Icons) per CSS
    von der Standardposition oben rechts nach unten rechts verschoben
    werden - so kollidiert sie nicht mehr mit Titeln oder der
    'Aktuell'-Markierung. Sie erscheint zudem nur beim Hover, bleibt
    aber vollständig nutzbar (Zoom, Pan, Download etc.).
    """
    ...
    """
    components.html(wrapped_html, height=height + 25, scrolling=False)
...
def render_stock_detail_page(ticker, benchmark_df):
    ...
    try:
        with st.spinner(f"Lade Detailanalyse für '{ticker}'..."):
            ...
            d_is_top = check_stan_weinstein_top_candidate(d_price, d_sma, d_slope, d_mrs, d_vol, d_vol_avg)
            ...
            with st.container(border=True):
                ...
                fig_d.update_layout(
                    height=520, margin=dict(l=10, r=20, t=30, b=25), showlegend=True,
            ...
            with st.expander("❓ Was bedeuten diese Kennzahlen?"):
                st.write(
                    "**Mansfield RS** misst die relative Stärke zum S&P 500 (> 0 = stärker als der Markt). "
                    "**30W-SMA** ist der 30-Wochen-Durchschnitt. **⭐ TOP KANDIDAT** bedeutet: Kurs über 30W-SMA, "
                    "Steigung > +0,5%, Mansfield RS > 0 und Volumen ≥ 120% des 10-Wochen-Schnitts (Stan-Weinstein-Stage-2-Setup)."
                )
        ...
        with st.container(border=True):
            ...
            with exp_header_col1:
                ...
                with expander_target:
                    ...
                    if stock_data:
                        stock_df = pd.DataFrame(stock_data).sort_values(by="Mansfield-RS", ascending=False).reset_index(drop=True)
                        ...
                        if selection_event and selection_event.get("selection", {}).get("rows"):
                            ...
                            sel_ticker = stock_df.iloc[sel_row]["Handelszeichen"]
