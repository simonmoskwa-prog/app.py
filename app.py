...
st.markdown(
    ...
    <div class="update-banner">
        ...
        <div class="update-banner-text">
            ...
            <strong>Live-Aktien-Analyse & Suche:</strong> Gib ab sofort ein beliebiges Ticker-Symbol ein, um in Echtzeit die Phasen-Auswertung (Stage 1–4), den 30W-SMA Trend und die Mansfield Relative Stärke für jede Aktie weltweit zu analysieren!
        </div>
    </div>
    """,
    unsafe_allow_html=True
...
def get_data_with_info(ticker):
    ...
    try:
        ...
        if (today_date - last_date).days <= 2 and today_date.weekday() < 5:
            ...
            if "DE" in ticker or "STX" in ticker or "DAX" in ticker or "WA" in ticker:
...
with st.container(border=True):
    ...
    with search_col1:
        search_query = st.text_input(
            "Aktiensymbol eingeben (z.B. AAPL, NVDA, TSLA, DINO.WA, PLTR, MSFT):",
            value="NVDA",
    ...
    if search_query:
        try:
            with st.spinner(f"Lade Daten für {search_query}..."):
                stock_df, stock_curr, stock_open, comp_name = get_data_with_info(search_query)
                ...
                with header_s1:
                    st.markdown(f"### {comp_name} (`{search_query}`)")
                    st.markdown(
                ...
                mrs_delta_color = "normal" if s_mrs >= 0 else "inverse"
            ...
            st.error(f"Aktie '{search_query}' konnte nicht gefunden oder geladen werden. Bitte überprüfe das Symbol (z.B. Ticker an der US-Börse oder mit Endung wie '.DE' oder '.WA').")
