...
for idx, item in enumerate(sector_results):
    ...
    with col:
        with st.container(border=True):
            ...
            # Nebeneinander-Layout: Expander-Titel links, Hilfebutton rechts in der gleichen Zeile
            exp_header_col1, exp_header_col2 = st.columns([0.92, 0.08])
            
            with exp_header_col2:
                if st.button("❓", key=f"help_icon_{item['ticker']}_{idx}", help="Hilfeseite & Glossar öffnen"):
                    show_glossary_dialog()
            ...
            with exp_header_col1:
                ...
                with expander_target:
                    ...
                    if stock_data:
                        ...
                        st.dataframe(
                            stock_df[["Top", "Name", "Ticker", "Kurs", "30W-SMA", "SMA-Steigung", "Mansfield-RS", "Status"]],
                            hide_index=True,
                            use_container_width=True,
                            column_config={
                                "Top": st.column_config.TextColumn(
                                    "Top",
                                    help="⭐ TOP = Aktie erfüllt alle 4 Kriterien nach Stan Weinstein (Preis > 30W-SMA, Steigung > +0.5%, Mansfield RS > 0, Volumen >= 120%). Klicke oben rechts auf ❓ für das vollständige Regelwerk."
                                )
                            }
                        )
                    else:
