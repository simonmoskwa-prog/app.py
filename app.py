...
def show_glossary_dialog():
    ...
    *Aktien unter dem 30W-SMA oder in einem Abwärtstrend werden in den Tabellen ausgeblendet.*
    ...
    2. **Dynamische Steigung (`> +0.5%`):** Die 4-Wochen-Steigung des 30W-SMA zeigt mit mehr als +0,5% eine solide Aufwärtsdynamik.
    3. **Outperformance (`Mansfield RS > 0.0`):** Die Aktie ist stärker als der Gesamtmarkt (S&P 500 Benchmark).
    4. **Volumen-Bestätigung (`Volumen >= 120% des 10W-Schnitts`):** Das Handelsvolumen liegt mindestens 20% über dem Durchschnitt der letzten 10 Wochen (institutioneller Rückenwind).
    """)
    ...
    st.markdown("""
    * **Stage 1 (Bodenbildung / Consolidation):** Der Kurs pendelt um einen flachen 30W-SMA. Akkumulationsphase.
    * **Stage 2 (Aufstiegsphase / Advancing Stage):** Kurs bricht über den 30W-SMA aus, der SMA steigt dynamisch an. **Das optimale Kauf-Fenster!**
    * **Stage 3 (Topbildung / Distribution):** Der Kurs verliert an Schwung und schwankt flach um einen abflachenden 30W-SMA.
...
def get_batch_stock_data(tickers):
    ...
    for t in tickers:
        try:
            ...
            df_t["SMA30_Slope_4W"] = ((df_t["SMA30"] - df_t["SMA30"].shift(4)) / df_t["SMA30"].shift(4)) * 100
...
# ---------------------------------------------------------
# MANSFIELD RS BERECHNUNG & STAGE LOGIK
# ---------------------------------------------------------
...
def determine_stage(price, sma30, slope):
    """
    Klassische Stufenbestimmung nach Stan Weinstein basierend auf Wochenkurs und 30W-SMA Steigung.
    """
...
def check_stan_weinstein_top_candidate(price, sma30, slope, mansfield_rs, volume=None, vol_avg_10w=None):
    """
    Prüft, ob eine Aktie alle Kriterien für ein erstklassiges Stage-2-Breakout-Setup erfüllt:
    1. Preis > 30W-SMA
    2. 30W-SMA Steigung > +0.5% (dynamisch ansteigend)
    3. Mansfield RS > 0.0 (Outperformance ggü. Benchmark)
    4. Volumen >= 120% des 10W-Durchschnitts (falls Volumendaten vorhanden)
    """
    ...
    try:
        with st.spinner(f"Lade Detailanalyse für '{ticker}'..."):
            ...
            with st.expander("❓ Was bedeuten diese Kennzahlen?"):
                st.write(
                    "**Mansfield RS** misst die relative Stärke zum S&P 500 (> 0 = stäker als der Markt). "
                    "**30W-SMA** ist der 30-Wochen-Durchschnitt im Wochenchart. **⭐ TOP KANDIDAT** bedeutet: Kurs über 30W-SMA, "
