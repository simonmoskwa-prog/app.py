...
st.caption("Markt → Sektor → Top-Aktien | Basierend auf Stan Weinsteins Phasenanalyse (30-Wochen-SMA & Mansfield Relative Strength)")
...
for idx, item in enumerate(sector_results):
    ...
    with col:
        with st.container(border=True):
            ...
            # Mini-Chart für den Sektor (mit Achsenbeschriftung)
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=item['df'].index[-52:], y=item['df']['Close'].iloc[-52:], mode='lines', name='Kurs', line=dict(color='blue', width=1.5)))
            fig.add_trace(go.Scatter(x=item['df'].index[-52:], y=item['df']['SMA30'].iloc[-52:], mode='lines', name='30W-MA', line=dict(color='orange', width=1.5)))
            fig.update_layout(
                height=180, 
                margin=dict(l=10, r=10, t=10, b=10), 
                showlegend=False,
                xaxis=dict(title="Datum", visible=True),
            ...
            with st.expander("🔍 Top-Aktien im Stage-2-Setup anzeigen"):
                ...
                if stock_data:
                    ...
                    st.dataframe(
                        stock_df[['Name', 'Ticker', 'Kurs', '30W-MA', 'Steigung', 'Mansfield-RS']], 
                        hide_index=True, 
                        use_container_width=True
                    )
                    
                    # Interaktive Chart-Auswahl
                    st.markdown("---")
                    ...
                    if selected_ticker:
                        ...
                        # Detail-Chart für die ausgewählte Aktie (mit vollen Achsenbeschriftungen)
                        st.caption(f"Wochen-Chart: **{selected_name}** (Blau = Kurs | Orange = 30W-MA)")
                        fig_stock = go.Figure()
                        fig_stock.add_trace(go.Scatter(x=s_df.index[-52:], y=s_df['Close'].iloc[-52:], mode='lines', name='Kurs', line=dict(color='#1f77b4', width=2)))
                        fig_stock.add_trace(go.Scatter(x=s_df.index[-52:], y=s_df['SMA30'].iloc[-52:], mode='lines', name='30W-MA', line=dict(color='#ff7f0e', width=2)))
                        fig_stock.update_layout(
                            ...
                            xaxis=dict(title="Datum", visible=True),
                            yaxis=dict(title="Preis ($)", visible=True)
                        )
