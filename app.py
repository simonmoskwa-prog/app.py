...
def get_data_with_info(ticker):
    ...
    try:
        ...
        # Zeitprüfung auf Basis lokaler deutscher Uhrzeit
        now_local = datetime.datetime.now()
        is_weekday = now_local.weekday() < 5  # 0=Montag bis 4=Freitag
        
        # Umrechnung der aktuellen Zeit in Minuten seit Mitternacht für präzisen Vergleich
        current_minute_of_day = now_local.hour * 60 + now_local.minute

        if is_weekday:
            # Liste aller europäischen Ticker-Kennungen & Symbole
            eu_identifiers = ["GDAXI", "STOXX", "DE", "STX", "WA", "PA", "F"]
            
            is_european = any(identifier in ticker for identifier in eu_identifiers)
            
            if is_european:
                # Europäische Handelszeiten: 09:00 Uhr (540 Min) bis 17:30 Uhr (1050 Min)
                is_open = 540 <= current_minute_of_day <= 1050
            else:
                # US-Handelszeiten: 15:30 Uhr (930 Min) bis 22:00 Uhr (1320 Min)
                is_open = 930 <= current_minute_of_day <= 1320
        else:
            is_open = False
