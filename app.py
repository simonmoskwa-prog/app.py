...
def get_data(ticker):
    # Stan Weinstein nutzt Wöchentliche Daten (1wk) über einen mehrjährigen Zeitraum
    df = yf.download(ticker, period="3y", interval="1wk")
    if isinstance(df.columns, pd.MultiIndex):
        df = df.xs(ticker, level=1, axis=1)
    df = df[['Close', 'Volume']].dropna()
    # 30-Wochen Moving Average (Standard nach Weinstein)
    ...
    with col:
        with st.container(border=True):
            ...
            fig.update_layout(
                ...
                xaxis=dict(title="Datum (Wochen)", visible=True),
            ...
            with st.expander("🔍 Top-Aktien im Stage-2-Setup anzeigen"):
                ...
                for stk_ticker, stk_name in item['stocks'].items():
                    try:
                        ...
                        # Filter nach Weinstein Stage 2 (Kurs > 30W-SMA, SMA steigend, RS positiv)
                ...
                if stock_data:
                    ...
                    if selected_ticker:
                        ...
                        s_plot_df = s_df.tail(104) # 2 Jahre Wochenbasis
                        
                        # Subplots: Oben Preis + 30W-SMA, unten Mansfield RS
                        ...
                        # 2. Mansfield Relative Stärke Indikator
                        fig_stock.add_trace(go.Scatter(
                        ...
                        # Weinstein Zero Line (Nulllinie für Mansfield RS)
                        fig_stock.add_trace(go.Scatter(
                        ...
                        fig_stock.update_xaxes(title_text="Datum (Wochen)", row=2, col=1)
