...
# ---------------------------------------------------------
# HEADER SECTION (INKL. SPRACHAUSWAHL & UPDATETEXT)
# ---------------------------------------------------------
...
with header_col3:
    st.write("")
    lang_col, btn_col1, btn_col2 = st.columns([1.2, 1, 1])
    with lang_col:
...
# Aktualisierter Update-Banner mit Informationen zum Long Term Halter Bereich
st.markdown(
    """
    <div class="update-banner">
        ...
        <div class="update-banner-text">
            ...
            <strong>Neues Release:</strong> Neben dem optimierten <strong>⭐ TOP Candidate System</strong> für Stage-2-Ausbrüche steht ab sofort die brandneue <strong>Wertanalyse für Long Term Halter</strong> zur Verfügung! Analysiere fundamentale Qualitätskennzahlen & Fair-Value-Modelle direkt im zweiten Tab.
        </div>
    </div>
    """,
    unsafe_allow_html=True
...
with tab_long:
    ...
    with st.container(border=True):
        ...
        if lt_query_to_run:
            ...
            if lt_data_ok:
                with st.spinner(f"Berechne Fundamental- und Bewertungsdaten für '{lt_name}'..."):
                    q_metrics = compute_quality_growth_metrics(lt_resolved_ticker)
                    
                    min_ret = lt_min_return_pct / 100.0
                    safety_m = (100 - lt_safety_margin_pct) / 100.0
                    val_res = compute_target_valuation(
                        q_metrics,
                        min_return=min_ret,
                ...
                st.markdown(
                    f"<div style='margin-top: 4px; margin-bottom: 16px;'>"
                    f"<span class='{val_res[\"verdict_class\"]}'>{val_res[\"verdict\"]}</span> &nbsp; {lt_status_badge} &nbsp; "
                    f"<span style='font-size: 0.85rem; color:#94a3b8;'>Börse: <b>{lt_exchange}</b> | Handelszeichen: <b>{lt_resolved_ticker}</b> | Währung: <b>{lt_curr}</b></span></div>",
                    unsafe_allow_html=True,
                )

                # Row 1: Key Pricing & Targets
                c1, c2, c3, c4 = st.columns(4)
                c1.metric("Aktueller Kurs", format_metric_money(q_metrics["current_price"], lt_curr))
                c2.metric("Rechnerischer Zielwert", format_metric_money(val_res["target_value"], lt_curr))
                c3.metric(f"Wert mit {lt_safety_margin_pct}% Abschlag", format_metric_money(val_res["safety_value"], lt_curr))
                payback_str = f"{val_res['payback_years']} Jahre" if val_res["payback_years"] else "n. v. (>30 J.)"
                c4.metric("Payback Period (Amortisation)", payback_str)

                st.markdown("---")
                st.markdown("#### 📊 Fundamentale Qualitäts- & Wachstumskennzahlen")

                # Row 2: Quality metrics
                r_st, r_tag = classify_growth_metric(q_metrics["roic_avg"], 0.12)
                e_st, e_tag = classify_growth_metric(q_metrics["eps_cagr"], 0.08)
                s_st, s_tag = classify_growth_metric(q_metrics["sales_cagr"], 0.08)
                eq_st, eq_tag = classify_growth_metric(q_metrics["equity_cagr"], 0.08)
                f_st, f_tag = classify_growth_metric(q_metrics["fcf_cagr"], 0.08)

                qcol1, qcol2, qcol3, qcol4, qcol5 = st.columns(5)
                with qcol1:
                    st.markdown(render_metric_card("ROIC / ROE Ø", format_metric_pct(q_metrics["roic_avg"]), r_st, r_tag), unsafe_allow_html=True)
                with qcol2:
                    st.markdown(render_metric_card("Gewinn-Wachstum (EPS)", format_metric_pct(q_metrics["eps_cagr"]), e_st, e_tag), unsafe_allow_html=True)
                with qcol3:
                    st.markdown(render_metric_card("Umsatz-Wachstum", format_metric_pct(q_metrics["sales_cagr"]), s_st, s_tag), unsafe_allow_html=True)
                with qcol4:
                    st.markdown(render_metric_card("Eigenkapital-Wachstum", format_metric_pct(q_metrics["equity_cagr"]), eq_st, eq_tag), unsafe_allow_html=True)
                with qcol5:
                    st.markdown(render_metric_card("Free Cash Flow-Wachstum", format_metric_pct(q_metrics["fcf_cagr"]), f_st, f_tag), unsafe_allow_html=True)

                st.write("")

                # Row 3: Model Assumptions
                with st.container(border=True):
                    st.markdown("##### ⚙️ Modell-Parameter & Zwischenergebnisse")
                    pcol1, pcol2, pcol3, pcol4 = st.columns(4)
                    pcol1.metric("Verwendetes EPS (TTM)", format_metric_money(q_metrics["eps_ttm"], lt_curr))
                    pcol2.metric("Angenommenes Wachstumsrate (p.a.)", format_metric_pct(val_res["growth_used"]))
                    pcol3.metric(f"Erwartetes EPS in {lt_forecast_years} Jahren", format_metric_money(val_res["future_eps"], lt_curr))
                    pcol4.metric("Angenommenes Ziel-KGV", f"{val_res['assumed_future_pe']:.1f}" if val_res["assumed_future_pe"] else "n. v.")

            else:
                st.error(f"Daten für '{lt_query_to_run}' (gefundener Ticker: '{lt_resolved_ticker}') konnten nicht geladen werden. Bitte überprüfe die Eingabe.")
