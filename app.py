...
for idx, item in enumerate(sector_results):
    ...
    with col:
        with st.container(border=True):
            ...
            # Nebeneinander-Layout: Expander links, Hilfebutton rechts daneben
            exp_col1, exp_col2 = st.columns([0.88, 0.12])
            
            with exp_col2:
                if st.button("❓", key=f"help_btn_{item['ticker']}_{idx}", help="Wann taucht eine Einzelaktie in dieser Liste auf?"):
                    show_glossary_dialog()
