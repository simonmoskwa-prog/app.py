...
# ---------------------------------------------------------
# AUTHENTICATION (SCHRITT 2)
# ---------------------------------------------------------
# Standard SHA-256 Hash für das Passwort "pulse2026"
# (Kannst du durch deinen eigenen Hash oder st.secrets["PASSWORD_HASH"] ersetzen)
PASSWORD_HASH = "fa2e022efef92f254e423efae5d787d5583b3e64c3c3a4f6be5994f2910fa3bd"
...
def render_login_screen():
    ...
    with col2:
        ...
        with st.container(border=True):
            ...
            st.caption("Bitte Passwort eingeben, um auf das Stage-Analysis RS Dashboard zuzugreifen.")
            
            pwd_input = st.text_input("Passwort", type="password", key="login_pwd")
            login_btn = st.button("Anmelden", use_container_width=True)
            
            if login_btn:
                if verify_password(pwd_input):
                    st.session_state["authenticated"] = True
                    st.success("Erfolgreich angemeldet!")
                    st.rerun()
                else:
                    st.error("Ungültiges Passwort. Zugriff verweigert.")

if not st.session_state["authenticated"]:
    render_login_screen()
    st.stop()

# Logout in der Sidebar anzeigen, wenn angemeldet
with st.sidebar:
    st.markdown("### 👤 Benutzer-Session")
    st.caption("Status: Angemeldet")
