...
def hash_password(password: str) -> str:
    ...
    salted_pwd = str(password) + get_salt()
...
# ---------------------------------------------------------
def load_users():
    """Lädt Benutzer dynamisch aus Secrets und optional aus der lokalen users.json."""
    users_db = {}

    # 1. Aus Streamlit Secrets laden (Struktur: [users.admin] & [passwords])
    if "users" in st.secrets and "passwords" in st.secrets:
        try:
            users_sec = st.secrets["users"]
            passwords_sec = st.secrets["passwords"]
            for u_key in users_sec:
                u_key_clean = str(u_key).strip().lower()
                info = users_sec[u_key]
                raw_pwd = passwords_sec.get(u_key, "")
    ...
    # 2. Ergänzend aus lokaler JSON-Datei laden (falls vorhanden)
    ...
    # 3. Direkter Fallback auf ADMIN_PASSWORD in st.secrets
...
def save_users(users_dict):
    try:
        ...
        st.warning(
            f"Benutzerdaten konnten nicht lokal gespeichert werden: {e}"
        )
    ...
    with col2:
        ...
        with st.container(border=True):
            ...
            with st.form("login_form"):
                ...
                if login_btn:
                    # Benutzer jedes Mal frisch laden
                    current_users = load_users()
                    st.session_state["users_db"] = current_users

                    if (
                        user_input in current_users
                        and current_users[user_input]["password"]
                        == hash_password(pwd_input)
                    ):
                        st.session_state["authenticated"] = True
                        st.session_state["user_key"] = user_input
                        st.session_state["username"] = current_users[user_input][
                            "display_name"
                        ]
                        st.session_state["role"] = current_users[user_input].get(
                            "role", "user"
                        )
                        st.session_state["welcome_msg"] = current_users[user_input][
                            "welcome_msg"
                        ]
                        st.success(
...
def show_glossary_dialog():
    ...
    * **2. Positiver Trend (`Steigung (4W) > 0.0%`):** Der 30-Wochen-Durchschnitt muss im Vergleich zu vor 4 Wochen ansteigen.
    ...
    3. **Outperformance (`Mansfield RS > 0,0`):** Die Aktie entwickelt sich stärker als der Gesamtmarkt (S&P 500).
        ...
        with st.expander("👥 Benutzer-Verwaltung", expanded=False):
            ...
            with tab_add:
                ...
                if st.button("User Anlegen", use_container_width=True):
                    db = st.session_state.get("users_db", load_users())
