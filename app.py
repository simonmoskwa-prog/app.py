...
# ---------------------------------------------------------
# ROBUSTE MANSFIELD RS BERECHNUNG (OHNE NaN FEHLER)
# ---------------------------------------------------------
def calculate_mansfield_rs(asset_df, benchmark_df):
    ...
    # Zeitzonen entfernen und auf reine Datumsangaben normieren
    asset_close.index = pd.to_datetime(asset_close.index).tz_localize(None).normalize()
    bench_close.index = pd.to_datetime(bench_close.index).tz_localize(None).normalize()

    # Gemeinsamer DataFrame zur Vermeidung von Lücken bei unterschiedlichen Kalendern
    df_combined = pd.DataFrame({"Asset": asset_close, "Bench": bench_close}).ffill().bfill()

    # Relative Stärke (R)
    base_rs = df_combined["Asset"] / df_combined["Bench"]

    # 52-Wochen-Durchschnitt mit min_periods=1 für Lückenlosigkeit
    rs_sma52 = base_rs.rolling(window=52, min_periods=1).mean()

    # Mansfield RS Formel: ((R / SMA(R)) - 1) * 100
    mansfield = ((base_rs / rs_sma52) - 1) * 100

    # Auf Index der Aktie reindexieren und verbleibende NaNs abfangen
    mansfield = mansfield.reindex(asset_close.index).ffill().bfill().fillna(0)
...
def check_stan_weinstein_top_candidate(price, sma30, slope, mansfield_rs):
    """
    Stan Weinstein Top Kandidat Kriterien:
    1. Kurs über dem 30W-SMA
    2. 30W-SMA steigt (Steigung > +0.5%)
    3. Outperformance gegenüber dem Markt (Mansfield RS > 0)
    """
    ...
    if search_query:
        ...
        try:
            with st.spinner(f"Lade Daten für '{search_query}' (Ticker: {resolved_ticker})..."):
                ...
                stock_df["Mansfield_RS"] = calculate_mansfield_rs(stock_df, bench_df).fillna(0)
                ...
                # INTERAKTIVER CHART MIT VOLUMEN UND MANSFIELD RS
        ...
        try:
            ...
            with st.container(border=True):
                ...
                with col_right:
                    ...
                    fig_idx = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.08, row_heights=[0.7, 0.3])
                    ...
                    fig_idx.update_layout(
                        height=260, margin=dict(l=10, r=20, t=15, b=25), showlegend=False,
                        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", hovermode="x unified",
...
for sec_name, sec_info in SECTOR_COMPONENTS.items():
    try:
        ...
        sec_df["Mansfield_RS"] = calculate_mansfield_rs(sec_df, bench_df).fillna(0)
...
for idx, item in enumerate(sector_results):
    ...
    with col:
        with st.container(border=True):
            ...
            # SUBPLOT: Preis + Volume + Mansfield RS
            ...
            with st.expander("🔍 Top-Aktien im Stage-2-Setup anzeigen"):
                ...
                for stk_ticker, stk_name in item["stocks"].items():
                    try:
                        ...
                        stk_df["Mansfield_RS"] = calculate_mansfield_rs(stk_df, bench_df).fillna(0)
